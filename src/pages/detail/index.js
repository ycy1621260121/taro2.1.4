...
componentWillUnmountn() {
  ...
  setImagePrew=(images,val,index,idx)=>{
    const imagesData = [];
    for(let i in images){
      imagesData[i]= images[i].image_url;
    }
    //console.log(imagesData,index);
    Taro.previewImage({
      current: imagesData[index], // 当前显示图片的http链接
      urls: imagesData // 需要预览的图片http链接列表
    })
  }
                      ...
                      {item.images &&
                        item.images.map((sub1, subIndex1) => (
                          <Image
                            ...
                            className="goods-img"
                            mode="widthFix"
                            onClick={this.setImagePrew.bind(this,item.images,sub1,subIndex1)}
                            src={sub1.image_url}
                          />
                        ))}
                    </View>
